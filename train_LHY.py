...
import sys
import shutil
from data.modelnet40_dataset import ModelNetDataset
...
NUM_CLASSES = 40
MODEL = importlib.import_module("geo_cnn_classification_LHY")  # import network module
...
model_file_src = os.path.join(Base_dir, "models","geo_cnn_classification_LHY.py")
model_file_dst = os.path.join(Log_dir, "geo_cnn_classification_LHY.py")
train_file_src = os.path.join(Base_dir, "train_LHY.py")
train_file_dst = os.path.join(Log_dir, "train_LHY.py")
copyfile(model_file_src, model_file_dst)
copyfile(train_file_src, train_file_dst)

assert (NUM_POINT <= 10000)
DATA_PATH = os.path.join(Data_dir, 'modelnet40_normal_resampled')
TRAIN_DATASET = ModelNetDataset(root=DATA_PATH, npoints=NUM_POINT, split='train',
                                                 normal_channel=True, batch_size=BATCH_SIZE, shuffle=None)
TEST_DATASET = ModelNetDataset(root=DATA_PATH, npoints=NUM_POINT, split='test',
                                                normal_channel=True, batch_size=BATCH_SIZE, shuffle=None)
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch % 10 == 0:
                save_path = saver.save(sess, os.path.join(Result_dir, str(epoch)+"model.ckpt"))
...
def train_one_epoch(sess, ops, train_writer):
    ...
    while TRAIN_DATASET.has_next_batch():
        saver = tf.train.Saver()  # todo:saver
        ...
        if (batch_idx + 1) % 50 == 0:
            ...
            total_seen = 0
            loss_sum = 0
            # save_path = saver.save(sess, os.path.join(Result_dir, "model.ckpt"))  # todo: save
            # write_log("Model saved in file: %s" % save_path)
        batch_idx += 1
