...
class AggregateGpuOp: public OpKernel {   //实现该OP的一个kernel，从后面的定义上可以看到，其规定只能在GPU上使用
  public:
    ...
    void Compute(OpKernelContext* context) override {
      ...
      auto feat_flat    = feat_tensor.flat<float>();  //flatten成一维
      auto xyz_flat     = xyz_tensor.flat<float>();

      const float* feat  = &(feat_flat(0));  //获取首地址
      const float* xyz   = &(xyz_flat(0));
      //op_requires 检查输入的合法性
      ...
      //获取feat的b，n，c三个dim的数值
      int b = feat_tensor.shape().dim_size(0);  //batch size
      int n = feat_tensor.shape().dim_size(1);  //每个数据包含的点数
      int c = feat_tensor.shape().dim_size(2) / 6;  //除以6这个操作就很迷惑

      Tensor* out_tensor = NULL;
      Tensor* norm_tensor = NULL;
      //给out_tensor norm_tensor分配输出内存
...
REGISTER_KERNEL_BUILDER(Name("Aggregate").Device(DEVICE_GPU), AggregateGpuOp)  //定义一个kernel，并且规定仅仅能在GPU上使用
