...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    l1_points_1 = mlp_knn(k=16,xyz=l0_xyz, points=l0_points, is_training=is_training, bn_decay=bn_decay, scope='l1_branch_1',
                          pooling='max')  # b*n*384
    # logger.debug("l1_points_1:", l1_points_1.shape)
    # ==========================layer1_branch_2====================================
    # Set abstraction layers
    l1_points_2 = geoconv(feat=l0_points, xyz=l0_xyz, num_outputs=128, bypass_num_outputs=64, radius=0.05, decay_radius=0.15,
                          bn=True, is_training=is_training, scope='geoconv_1', bn_decay=bn_decay, activation_fn=tf.nn.relu,
                          delta=False, dropout_self=0, dropout_edge_1=0, dropout_dege_2=0.3)
    # todo：dropout: 由于在比较靠近输入的地方进行dropout和数据增广十分接近，然而数据增广对训练又有比较坏的影响，因此选在在比较靠后的地方进行drop==》geovonv的第二个mlp
    ...
    # logger.debug("l2_points:", l2_points.shape)
    l2_points = tf.layers.dropout(l2_points, rate=0.3)
    # todo：dropout: 在从第三层geoconv进行全连接转到2048维的时候，768*2048全连接参数量较大，容易过拟合
